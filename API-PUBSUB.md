...
project. If both primary and secondary keys are included, they must be
from the same Cogswell Pub/Sub project. In addition, the url for the host
should be provided. If the host is not provided, it defaults to 
`wss://api.cogswell.io/pubsub`. Finally, in order to actually run
the integration tests, run the command `./gradlew cleanTest itest`
from the `cogs-java-client-sdk` directory. 
